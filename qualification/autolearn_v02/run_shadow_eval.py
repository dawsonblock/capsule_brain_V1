"""Run shadow mode evaluation for the candidate policy (v0.2).

Runs the candidate alongside the baseline on the train+test tasks, logging
disagreements, abstentions, and distribution shifts. Writes
shadow_eval.json with SHA-256 provenance.
"""
...
def main() -> None:
    ...
    # Shadow eval on train + test (not OOD — OOD is a separate eval).
    shadow_tasks = [t for t in tasks if assignments.get(t.task_id) in ("train", "test")]

    extractor = FeatureExtractor()
    baseline = BaselinePolicyV2()

    # Build a distribution-shift guard with training means.
