"""Real per-action isolated service factory + task provisioners for v0.3.2.

Section 6/7: every task/action execution receives its own isolated service
bundle (fresh MemoryService, ExperienceStore, ToolRegistry, WorkflowRunner,
ConversationService, ExecutiveController, isolated DBs + filesystem). The
provisioners mutate those real services with task-specific state:

- memory: secret written ONLY through the isolated MemoryService; never in
  the prompt/features/model-visible metadata.
- tool: a real benchmark tool handler registered in the isolated ToolRegistry;
  output generated at execution time; invocation counter maintained.
- workflow: isolated workspace + acceptance test installed; the real workflow
  + acceptance execution runs.
- safety: dangerous request preempted by ExecutiveSafetyGuard before the
  learned policy.
...
class QualificationServices:
    ...
    async def execute_action(
        ...
        *,
        ...
        # Build the request context with task setup (model-visible subset only;
        # secrets/expected outputs are NOT included here — they live in
        # verifier_state on the provisioned task).
    ...
    def _model_visible_metadata(self, task: dict[str, Any]) -> dict[str, Any]:
        """Metadata handed to the runtime. Never includes secrets/expected
        outputs. For workflow tasks, the acceptance_code is included so the
        workflow path can install the acceptance contract (the acceptance test
        is a verification asset, not a hidden answer the model must produce)."""
        md = {
            "text": task["prompt"],
        ...
        for key in ("secret", "expected_tool_output", "expected_token", "nonce",
                    "solution_code", "conflict_secret"):
            setup.pop(key, None)
        # Workflow acceptance contract is a verification asset, not a hidden
        # model answer; the workflow path needs it to run the acceptance test.
...
class QualificationServiceFactory:
    ...
    def __init__(self, config, *, sandbox_root: str) -> None:
        self.config = config
    ...
    async def _provision(
        ...
        if family == "workflow_required":
            # Acceptance test is installed by the conversation workflow path via
            # the request context metadata. Nothing to mutate here beyond
            # recording verifier state.
            return ProvisionedTask(
                task_id=task["task_id"],
                family=family,
                verifier_state={
        ...
        return ProvisionedTask(
            ...
            family=family,
            verifier_state={
                "expected_token": setup.get("expected_token", ""),
            },
