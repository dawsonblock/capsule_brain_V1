...
Consumes real_counterfactual_results.json, groups by task, selects the
argmax action per task, and writes a versioned TrainingExample dataset.

Only rows with runtime_type='real' and action_completed=True are eligible.
Rows with not_executed or runtime_error get utility -1000 and are excluded.
"""
...
from .schemas import canonical_json, read_json, sha256_text, write_json
...
def _best_and_second(utilities: list[tuple[Action, float]]) -> tuple[Action, float, float]:
    ...
    u_second = sorted_u[1][1] if len(sorted_u) > 1 else 0.0
...
def build_dataset(config: QualificationConfig) -> dict[str, Any]:
    ...
    if not rows_path.exists():
        raise RuntimeError(f"counterfactual results missing: {rows_path}")
    rows = read_json(rows_path)

    bm = read_json(artifacts / "benchmark_manifest.json")
    task_by_id = {t["task_id"]: t for t in bm["tasks"]}

    # Filter only real, completed rows.
    eligible = [
        r for r in rows
        if r.get("runtime_type") == "real"
        and r.get("outcome", {}).get("action_completed", False)
        and not r.get("not_executed", False)
    ]
    ...
    for task_id, task_rows in by_task.items():
        ...
        # Experience quality.
        runtime_rows = [r for r in task_rows if r["action"] == best.value]
        if runtime_rows:
            q = compute_experience_quality(
                runtime_type="real",
                verifier_type=runtime_rows[0].get("verification_evidence", {}).get("verifier_type", "deterministic"),
                action_set_complete=len(task_rows) == 4,
                isolation_enforced=True,
            )
            q_scores.append(q.quality_score)

    # Split according to task split (experience/validation/test/ood/safety).
    # Safety and OOD examples are held-out; they are not used for training.
    ...
    # Also write human-readable split files.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.2 counterfactual dataset.")
