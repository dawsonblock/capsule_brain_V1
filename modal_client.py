...
Containers stay warm between calls, so there's no cold start after
the first request. This is the fast path for scientific qualification.
...
# Reference the deployed app by name.
app = modal.App.lookup("capsule-brain-qual-v04", create_if_missing=False)

# Look up the deployed provider class and standalone function.
FrozenTransformerProvider = modal.Cls.from_name(
    "capsule-brain-qual-v04", "FrozenTransformerProvider",
)
generate_single_fn = modal.Function.from_name(
...
class ModalGPIClient:
    ...
    have no cold start (~0.5-2s per generation instead of ~50s).
    """
    ...
    def __init__(
        ...
        # Get a handle to the deployed provider.
        self._provider = FrozenTransformerProvider(
            model_id=model_id,
    ...
    def batch_generate(self, prompts, max_new_tokens=256):
        ...
        Fast for small-medium batches (no container startup overhead).
        """
    ...
    def parallel_generate(self, prompts, max_new_tokens=256):
        ...
        Best for large batches (>20 prompts). Each prompt gets its own
        container call. With warm containers, this is both parallel AND
        cold-start-free.
        """
        import json
        kwargs = {
        ...
        return list(generate_single_fn.map(
            prompts,
            kwargs=kwargs,
            order_outputs=True,
        ))
...
if __name__ == "__main__":
    ...
    # Single generation (warm container).
    ...
    print(f"\nBatch of {len(prompts)} (warm container, sequential)...")
    start = time.time()
    ...
    # Second batch — should be even faster (container already warm).
    print(f"\nSecond batch of {len(prompts)} (warm, no cold start)...")
    start = time.time()
    results2 = client.batch_generate(prompts, max_new_tokens=32)
    elapsed2 = time.time() - start
    print(f"  Total: {elapsed2:.1f}s, avg: {elapsed2/len(prompts):.1f}s/prompt")
